...
namespace Match3
{
    public class GameGrid : MonoBehaviour
    {
        ...
        public void FreezeRandomPieces(int count)
        {
            ...
            {
                int x = UnityEngine.Random.Range(0, xDim);
                int y = UnityEngine.Random.Range(0, yDim);
        ...
        public void BossCenterPieces(int count)
        {
            int centerX = xDim / 2;
            int centerY = yDim / 2;

            // Определение координат для области 3x3 в центре доски
            int startX = Math.Max(0, centerX - 1);
            int startY = Math.Max(0, centerY - 1);
            int endX = Math.Min(xDim - 1, centerX + 1);
            int endY = Math.Min(yDim - 1, centerY + 1);

            int frozenCount = 0;

            for (int x = startX; x <= endX && frozenCount < count; x++)
            {
                for (int y = startY; y <= endY && frozenCount < count; y++)
                {
                    // Замените тип элемента соответствующим образом перед замораживанием его
                    if (_pieces[x, y].Type == PieceType.Normal)
                    {
                        ...
                        SpawnNewPiece(x, y, PieceType.Boss);
                        frozenCount++;
                    }
        ...
        public void ClearStonesAround(int centerX, int centerY)
        {
            // Определение координат для области 3x3 вокруг заданной позиции
            int startX = Math.Max(0, centerX - 3);
            int startY = Math.Max(0, centerY - 1);
            int endX = Math.Min(xDim - 1, centerX + 3);
            int endY = Math.Min(yDim - 1, centerY + 3);

            for (int x = startX; x <= endX; x++)
            {
                for (int y = startY; y <= endY; y++)
                {
                    // Если элемент является камнем, то заменить его на обычный элемент
                    if (_pieces[x, y].Type == PieceType.Normal)
                    {
                        // Уничтожьте текущий камень перед созданием нового обычного элемента
                        GameObject pieceToClear = _pieces[x, y].gameObject;
                        Destroy(pieceToClear);
        ...
        public void BossAdjacent(GamePiece piece)
        {
            for (int xOffset = -1; xOffset <= 1; xOffset++)
            {
                for (int yOffset = -1; yOffset <= 1; yOffset++)
                {
                    ...
                    if (newX >= 0 && newX < xDim && newY >= 0 && newY < yDim)
                    {
                        if (_pieces[newX, newY].Type == PieceType.Boss)
                        {
